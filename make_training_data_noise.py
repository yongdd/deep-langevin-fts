...
data_path = "data3d_gyroid_dis_noise_2"
...
for langevin_step in range(0, langevin_max_iter):
    ...
    if( langevin_step % recording_period_train == 0):
        ...
        for data in data_chosen:
            w_plus_diff = w_plus-data["w_plus"]
            data["w_plus"] += np.random.normal(0, np.std(w_plus_diff)/3, sb.get_n_grid())
            QQ = pseudo.find_phi(phi_a, phi_b,
                q1_init, q2_init,
                data["w_plus"] + w_minus,
                data["w_plus"] - w_minus)
            data["g_plus"] = phi_a + phi_b - 1.0
            save_data(data_path, "fields_%d" % np.round(chi_n*100), langevin_step, data["iter"], 
