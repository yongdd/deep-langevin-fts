...
class LitAtrousParallelImagePoolingMish(pl.LightningModule): # ASPP
    ...
    def forward(self, x):
        ...
        x1 = F.mish(self.bm4_1(self.conv_aspp4_1(x)))
        x2 = F.mish(self.bm4_2(self.conv_aspp4_2(x)))
        x3 = F.mish(self.bm4_3(self.conv_aspp4_3(x)))
        x4 = F.mish(self.bm4_4(self.conv_aspp4_4(x)))

        x_avg = self.avg_pool(x)
        x_avg = F.mish(self.bm4_5(self.conv_aspp4_5(x_avg)))
        x_avg = F.interpolate(x_avg, size=size, mode="trilinear", align_corners=False)
