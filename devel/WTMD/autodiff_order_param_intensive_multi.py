...
for i in range(1):
    ...
    w_minus_1 = torch.tensor(input_fields[0,:,:,:], requires_grad=True)*2
    # w_minus_2 = torch.tensor(input_fields[1,:,:,:], requires_grad=True)*2
    w_minus_2 = torch.rand(nx, requires_grad=True, dtype=torch.float64)*20
    w_minus_1_k = torch.fft.rfftn(w_minus_1)
    w_minus_2_k = torch.fft.rfftn(w_minus_2)
    
    Psi =       torch.sum(torch.pow(torch.absolute(w_minus_1_k), ell)*fk*wt)
    Psi = Psi + torch.sum(torch.pow(torch.absolute(w_minus_2_k), ell)*fk*wt)
    
    Psi = torch.pow(Psi/2, 1.0/ell)/M

    # print(w_minus_k[0,0,10].item(), ell, fk[0,0,10].item(), wt[0,0,10].item())

    # auto_grad_1 = torch.autograd.grad(Psi*M/V, w_minus_1)
    auto_grad_2 = torch.autograd.grad(Psi*M/V, w_minus_2)
    
    w_minus_1_k_np = w_minus_1_k.detach().cpu().numpy()
    w_minus_2_k_np = w_minus_2_k.detach().cpu().numpy()
    fk_np = fk.detach().cpu().numpy()
    Psi_np = Psi.detach().cpu().numpy()
    
    dPsi_dwk_1 = np.power(np.absolute(w_minus_1_k_np), ell-2.0) * np.power(Psi_np,1.0-ell)*w_minus_1_k_np*fk_np
    dPsi_dwk_2 = np.power(np.absolute(w_minus_2_k_np), ell-2.0) * np.power(Psi_np,1.0-ell)*w_minus_2_k_np*fk_np
    dPsi_dwr_1 = np.fft.irfftn(dPsi_dwk_1, m)*np.power(M, 2.0-ell)/V/2
    dPsi_dwr_2 = np.fft.irfftn(dPsi_dwk_2, m)*np.power(M, 2.0-ell)/V/2
    manual_func_deriv_1 = dPsi_dwr_1
    manual_func_deriv_2 = dPsi_dwr_2
    ...
    # print(manual_func_deriv_1[0,0,10], auto_grad_1[0][0,0,10])
    print(manual_func_deriv_2[0,0,10], auto_grad_2[0][0,0,10])
    # print(i, torch.std(auto_grad_1[0]-manual_func_deriv_1).item(), torch.std(auto_grad_1[0]), np.std(manual_func_deriv_1))
    print(i, torch.std(auto_grad_2[0]-manual_func_deriv_2).item(), torch.std(auto_grad_2[0]), np.std(manual_func_deriv_2))
    # print(i)
