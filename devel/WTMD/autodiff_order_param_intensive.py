...
# fk_2 = 1.0/(1.0 + np.exp(12.0*(mag_k-kc)/kc))

# wt_2 = np.zeros([m[0],m[1],m[2]//2+1])
# wt_2[:,:,:] = 2.0
# wt_2[:,:,0] = 1.0
# wt_2[:,:,m[2]//2] = 1.0

# fk_2 = np.reshape(fk_2, Mk)
# wt_2 = np.reshape(wt_2, Mk)

# print("np.std(fk_2-fk)", np.std(fk_2-fk))
# print("np.std(wt_2-wt)", np.std(wt_2-wt))
...
for i in range(1):
    ...
    Psi = torch.sum(torch.pow(torch.absolute(w_minus_k), ell)*fk*wt)
    Psi = torch.pow(Psi, 1.0/ell)/M
    ...
    dPsi_dwr = np.fft.irfftn(dPsi_dwk, m)*np.power(M, 2.0-ell)/V
